...
(() => {
  ...
  function renderMarkdown(text) {
    ...
    html = html.replace(
      /^- \[x\] (.+)$/gm,
      '<div class="task-item"><span class="task-checkbox checked"></span><del>$1</del></div>'
    );
    html = html.replace(
      /^- \[ \] (.+)$/gm,
      '<div class="task-item"><span class="task-checkbox"></span>$1</div>'
    );
  ...
  function createCardHtml(memo, index) {
    ...
    const tagsHtml = (memo.tags || [])
      .slice(0, 4)
      .map(t => `<span class="memo-tag">${escapeHtml(t)}</span>`)
      .join('');

    return `
      <article class="memo-card ${memo.status === 'completed' ? 'completed' : ''}"
               data-id="${memo.id}"
        ...
        <div class="memo-card-footer">
          <span class="memo-card-category ${memo.category}">
            ${CATEGORY_LABELS[memo.category] || memo.category}
          </span>
  ...
  function saveMemo() {
    ...
    if (editingId) {
      ...
      if (memo) {
        memo.title = title;
        memo.category = memoCategorySelect.value;
        memo.priority = memoPrioritySelect.value;
        memo.status = memoStatusSelect.value;
        memo.tags = tags;
        memo.content = memoContentInput.value;
        memo.updatedAt = now;
        toast('备忘已更新', 'success');
      }
    } else {
      memos.unshift({
        id: uuid(),
        title,
        category: memoCategorySelect.value,
        priority: memoPrioritySelect.value,
        status: memoStatusSelect.value,
        tags,
  ...
  function openDetail(id) {
    ...
    detailMeta.innerHTML = `
      <span class="detail-meta-item">
        <span class="memo-card-category ${memo.category}">${CATEGORY_LABELS[memo.category]}</span>
      </span>
      <span class="detail-meta-item">优先级：
        <span class="priority-badge ${memo.priority}">
          ${memo.priority === 'high' ? '高' : memo.priority === 'medium' ? '中' : '低'}
        </span>
      </span>
      <span class="detail-meta-item">状态：${
        memo.status === 'completed' ? '✅ 已完成' :
        memo.status === 'archived' ? '📦 已归档' : '🔄 进行中'
      }</span>
      ...
      ${(memo.tags || []).length ? `
        <span class="detail-meta-item">
          ${memo.tags.map(t => `<span class="memo-tag">${escapeHtml(t)}</span>`).join(' ')}
        </span>
      ` : ''}
    `;
  ...
  function handleToolbar(e) {
    ...
    switch (action) {
      ...
      case '~~':
        ...
        cursorOffset = sel ? insert.length : action.length;
        break;
      case 'heading':
        insert = `\n## ${sel || '标题'}`;
      ...
      case 'task':
        insert = `\n- [ ] ${sel || '任务'}`;
        cursorOffset = sel ? insert.length : 7;
        break;
      case 'code':
        insert = `\`${sel || '代码'}\``;
  ...
  function exportData() {
    ...
    a.href = url;
    a.download = `research_memos_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  function importData(file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        saveMemos();
        renderMemos();
        toast(`已导入 ${newMemos.length} 条备忘 (共 ${importCount} 条)`, 'success');
      } catch {
  ...
  function bindEvents() {
    ...
    $('#btnGridView').addEventListener('click', () => {
      viewMode = 'grid';
      memoGrid.classList.remove('list-view');
      $('#btnGridView').classList.add('active');
      $('#btnListView').classList.remove('active');
    });
    $('#btnListView').addEventListener('click', () => {
      viewMode = 'list';
      memoGrid.classList.add('list-view');
    ...
    document.addEventListener('keydown', (e) => {
      ...
      // Ctrl/Cmd + S: save in modal
      if ((e.ctrlKey || e.metaKey) && e.key === 's' && modalOverlay.classList.contains('open')) {
        e.preventDefault();
        saveMemo();
      }
    });
  ...
  function seedDemoData() {
    ...
    memos = [
      {
        id: uuid(), title: '蛋白质折叠模型优化思路',
        category: 'idea', priority: 'high', status: 'active',
        tags: ['深度学习', 'AlphaFold', '结构预测'],
        content: `# 优化方向\n\n## 模型结构\n- 引入 **SE(3)-Transformer** 改进等变性\n- 增加侧链建模的独立分支\n- 尝试 diffusion model 生成构象\n\n## 训练策略\n1. 使用对比学习预训练\n2. 多任务学习结合功能预测\n3. 数据增强：随机旋转 + 噪声扰动\n\n## 待验证\n- [ ] 在 CASP15 数据集测试 GDT-TS\n- [ ] 对比 ESMFold 结果\n- [x] 完成文献调研`,
        createdAt: now - 86400000, updatedAt: now - 3600000
      },
      {
        id: uuid(), title: 'PCR 实验条件优化',
        category: 'experiment', priority: 'medium', status: 'active',
        tags: ['分子克隆', 'PCR'],
        content: `## 实验目标\n扩增目标基因片段 (1.2kb)\n\n## 最优条件\n| 参数 | 值 |\n| --- | --- |\n| 退火温度 | 58°C |\n| 延伸时间 | 90s |\n| 循环数 | 32 |\n| 模板量 | 50ng |\n\n## 结果\n琼脂糖凝胶电泳结果显示 **单一条带**，大小正确。\n\n> 注意：Mg²⁺ 浓度从 1.5mM 调整到 2.0mM 后效果显著改善`,
        createdAt: now - 172800000, updatedAt: now - 86400000
      },
      {
        id: uuid(), title: 'Attention Is All You Need 笔记',
        category: 'literature', priority: 'medium', status: 'completed',
        tags: ['Transformer', 'NLP', '注意力机制'],
        content: `# Attention Is All You Need\n\n**Authors**: Vaswani et al. (2017)\n**DOI**: 10.48550/arXiv.1706.03762\n\n## 核心贡献\n- 提出了纯注意力架构 Transformer\n- 摒弃 RNN/CNN，完全基于 Self-Attention\n- 引入 Multi-Head Attention 和 Positional Encoding\n\n## 关键公式\n\`Attention(Q,K,V) = softmax(QK^T / sqrt(d_k)) V\`\n\n## 对我们工作的启发\n可以考虑将 cross-attention 应用到蛋白质-配体相互作用预测中`,
        createdAt: now - 259200000, updatedAt: now - 259200000
      },
      {
        id: uuid(), title: '本周科研待办',
        category: 'todo', priority: 'high', status: 'active',
        tags: ['周计划'],
        content: `- [x] 完成文献综述初稿\n- [ ] 跑模型消融实验\n- [ ] 准备组会 PPT\n- [ ] 修改论文 Introduction 部分\n- [ ] 联系合作者讨论数据共享`,
        createdAt: now - 43200000, updatedAt: now - 7200000
      },
      {
        id: uuid(), title: '课题组组会纪要 (6/12)',
        category: 'meeting', priority: 'low', status: 'active',
        tags: ['组会', '进展汇报'],
        content: `## 参会人员\n导师、全体研究生\n\n## 主要内容\n1. 张三汇报了分子动力学模拟进展\n2. 李四展示了新的实验结果\n3. 导师建议优先完成 **对比实验**\n\n## 分配任务\n- 我负责补充 baseline 实验\n- 下周五前提交实验报告\n\n---\n*下次组会：6/19（周四）*`,
        createdAt: now - 345600000, updatedAt: now - 345600000
      }
    ];
    saveMemos();
  }
